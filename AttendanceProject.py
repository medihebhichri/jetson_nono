...
def TextSpeach(name):
    ...
    if (compteur < 1):
        if (name[0] == "IHEB"):
            tts = gTTS(f"سَلاَمْ  , إيهَابْ. ْ؟", lang='ar')
            tts.save("hello.mp3")
            os.system("hello.mp3")
        if (name[0] == "HABIB"):
            tts = gTTS(f"سَلاَمْ  ,ْحَبِيبْ . ؟", lang='ar')
            tts.save("hello.mp3")
            os.system("hello.mp3")
...
while True:
    ...
    for encodeFace, faceLoc in zip(encodesCurFrame, facesCurFrame):
        ...
        if matches[matchIndex]:
            ...
            cv2.rectangle(img, (x1, y2 - 35), (x2, y2), (0, 255, 0, 7), cv2.FILLED)
...
"""speaker = pyttsx3.init()
  ...
  speaker.setProperty('voice', "com.apple.speech.synthesis.voice.sin-ji")
  speaker.setProperty('voice', "VoiceGenderMale")
  speaker.say(name[0])
